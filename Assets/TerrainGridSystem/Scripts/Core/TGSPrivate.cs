				...
				public partial class TerrainGridSystem : MonoBehaviour {
								...
								public void Init () {
												...
												if (hudMatCellOverlay == null) {
																hudMatCellOverlay = Instantiate (Resources.Load <Material> ("Materials/HudCellOverlayTex")) as Material;
																hudMatCellOverlay.hideFlags = HideFlags.DontSave;
												}
												if (hudMatCellGround == null) {
																hudMatCellGround = Instantiate (Resources.Load <Material> ("Materials/HudCellGroundTex")) as Material;
					...
					if (terrainRoughnessMap[l,c]<0.005f) {
						...
						marker.transform.localScale = Misc.Vector3one * 350/512.0f;
								...
								void UpdateMaterialNearClipFade () {
												...
												cellsMat.SetFloat ("_NearClip", nearClipFade);
												cellsMat.SetFloat ("_FallOff", nearClipFadeFallOff);
								...
								Material GetColoredTexturedMaterial (Color color, Texture2D texture, bool overlay) {
												...
												if (texture == null && matCache.ContainsKey (color)) {
																return matCache [color];
												} else {
																Material customMat;
																if (texture != null) {
																				Material mat = overlay ? texturizedMatOverlay : texturizedMatGround;
																				customMat = Instantiate (mat);
																				customMat.name = mat.name;
																				customMat.mainTexture = texture;
																} else {
																				Material mat = overlay ? coloredMatOverlay : coloredMatGround;
																				customMat = Instantiate (mat);
																				customMat.name = mat.name;
								...
								GameObject GenerateRegionSurface (Region region, int cacheIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace) {
												...
												GameObject surf = Drawing.CreateSurface (cacheIndexSTR, meshPoints.ToArray (), triNew, material, rect, textureScale, textureOffset, textureRotation, rotateInLocalSpace);
												_lastVertexCount += meshPoints.Count; // surf.GetComponent<MeshFilter> ().sharedMesh.vertexCount;
								...
								void UpdateHighlightFade () {
												...
												if (_highlightedObj != null) {
																...
																Color color = mat.color; // _highlightMode == HIGHLIGHT_MODE.Territories ? _territoryHighlightColor : _cellHighlightColor; // mat.color;
...
//				return transform.TransformPoint (new Vector2 (localPosition.x * _gridScale.x + _gridCenter.x, localPosition.y * _gridScale.y + _gridCenter.y));
								...
								GameObject HighlightTerritoryRegion (int territoryIndex, bool refreshGeometry) {
												...
												if (existsInCache) {
																...
																_highlightedObj = GenerateTerritoryRegionSurface (territoryIndex, hudMatTerritory, Vector2.one, Vector2.zero, 0, false);
												}
			...
			DestroySurfaces();
												...
												if (existsInCache) {
																...
																_highlightedObj = GenerateCellRegionSurface (cellIndex, hudMatCell, Vector2.one, Vector2.zero, 0, false);
												}
												// Reuse cell texture
