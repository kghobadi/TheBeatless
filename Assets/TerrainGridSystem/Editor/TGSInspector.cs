	...
	public class TGSInspector : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			if (tgs.territoriesTexture != null) {
				...
				tgs.territoriesTextureNeutralColor = EditorGUILayout.ColorField (tgs.territoriesTextureNeutralColor, GUILayout.Width (50));
			...
			tgs.gridSurfaceDepthOffset = EditorGUILayout.IntSlider (tgs.gridSurfaceDepthOffset, -20, 0);
			EditorGUILayout.EndHorizontal ();
			...
			if (!Application.isPlaying) {
				...
				if (cellSelectedIndex < 0 || cellSelectedIndex >= tgs.cells.Count) {
					...
					if (cellTag==selectedCell.tag) GUI.enabled = false;
					...
					if (cellTerritoryIndex == selectedCell.territoryIndex) GUI.enabled = false;
					...
					if (tgs.CellGetColor(cellSelectedIndex)==cellColor && tgs.CellGetTextureIndex(cellSelectedIndex) == cellTextureIndex) GUI.enabled = false;
		...
		void OnSceneGUI ()
		{
			if (tgs == null || Application.isPlaying)
				return;
			Event e = Event.current;
			tgs.CheckRay (HandleUtility.GUIPointToWorldRay (e.mousePosition));
